    ...
    <div className="flex min-h-screen items-center justify-center bg-gray-50 dark:bg-[#030308] p-4 text-gray-900 dark:text-white">
      <form className="glass-card w-full max-w-lg rounded-xl p-6" onSubmit={onSubmit}>
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2">
          ...
          <input type="email" className="rounded-md border border-white/20 bg-transparent px-3 py-2 md:col-span-2 text-gray-900 dark:text-white bg-gray-100 dark:bg-[#14141f] border-gray-200 dark:border-white/10 placeholder-gray-500 dark:placeholder-white/25" placeholder="Email" value={form.email} onChange={(e) => setForm((p) => ({ ...p, email: e.target.value }))} />
          <input type="password" className="rounded-md border border-white/20 bg-transparent px-3 py-2 md:col-span-2 text-gray-900 dark:text-white bg-gray-100 dark:bg-[#14141f] border-gray-200 dark:border-white/10 placeholder-gray-500 dark:placeholder-white/25" placeholder="Password" value={form.password} onChange={(e) => setForm((p) => ({ ...p, password: e.target.value }))} />
        </div>
        <button className="mt-4 w-full rounded-md bg-accent py-2 font-semibold" disabled={loading}>{loading ? "Saving..." : "Create Account"}</button>
        <p className="mt-4 text-sm">Already have account? <Link className="text-cyan" to="/login">Login</Link></p>
      </form>
    </div>
  );
}
